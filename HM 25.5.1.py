...
class TestShowMyPets:
    ...
    def setup(self):
        self.user = "simonova@gmail.com"
        self.password = "Simonova0701"
        self.open()


    def open(self):
        self.driver = webdriver.Chrome('D:\Skillfactory\PythonTests\Drivers\chromedriver.exe')
    ...
    def test_show_my_pets(self):
        ...
        images = self.driver.find_elements(By.CSS_SELECTOR, ('.card-deck .card-img-top')) # определяем фото
        names = self.driver.find_elements(By.CSS_SELECTOR, ('.card-deck .card-title')) # определяем имя
        descriptions = self.driver.find_elements(By.CSS_SELECTOR, ('.card-deck .card-text')) # определяем вид и возраст
